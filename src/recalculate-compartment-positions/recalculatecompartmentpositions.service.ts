...
export class RecalculateCompartmentPositionsService {
    RecalculateCompartmentPositions: ITempletu[] = [
        { id: '1', 
        lastName: "John",
        firstName: "Doe", 
    ...
    getRecalculateCompartmentPositions(): ITempletu[] {
        return this.RecalculateCompartmentPositions
    }

    getTempletu(id: string): ITempletu {
        const foundedTempletu = this.RecalculateCompartmentPositions.find(templetu => templetu.id === id)
        return foundedTempletu
    }

    createTempletu(templetuDTO: TempletuDTO): ITempletu {
        this.RecalculateCompartmentPositions.push(templetuDTO)
        return templetuDTO
    }
    ...
    deleteTempletu(id: string) {
        this.RecalculateCompartmentPositions = this.RecalculateCompartmentPositions.filter(templetu => templetu.id !== id)
    }
}
