...
export class CheckVehicleLoadingAdrService {
    CheckVehicleLoadingAdr: ITempletu[] = [
        { id: '1', 
        lastName: "John",
        firstName: "Doe", 
    ...
    getCheckVehicleLoadingAdr(): ITempletu[] {
        return this.CheckVehicleLoadingAdr
    }

    getTempletu(id: string): ITempletu {
        const foundedTempletu = this.CheckVehicleLoadingAdr.find(templetu => templetu.id === id)
        return foundedTempletu
    }

    createTempletu(templetuDTO: TempletuDTO): ITempletu {
        this.CheckVehicleLoadingAdr.push(templetuDTO)
        return templetuDTO
    }
    ...
    deleteTempletu(id: string) {
        this.CheckVehicleLoadingAdr = this.CheckVehicleLoadingAdr.filter(templetu => templetu.id !== id)
    }
}
