...
export class CheckPreloadingsService {
    ...
    getCheckPreloadings(): ITempletu[] {
        return this.CheckPreloadings
    }

    getTempletu(id: string): ITempletu {
        const foundedTempletu = this.CheckPreloadings.find(templetu => templetu.id === id)
        return foundedTempletu
    }

    createTempletu(templetuDTO: TempletuDTO): ITempletu {
        this.CheckPreloadings.push(templetuDTO)
        return templetuDTO
    }
    ...
    deleteTempletu(id: string) {
        this.CheckPreloadings = this.CheckPreloadings.filter(templetu => templetu.id !== id)
    }
}
