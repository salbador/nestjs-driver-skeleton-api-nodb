...
export class InitiateWeigingService {
    ...
    getInitiateWeiging(): ITempletu[] {
        return this.InitiateWeiging
    }

    getTempletu(id: string): ITempletu {
        const foundedTempletu = this.InitiateWeiging.find(templetu => templetu.id === id)
        return foundedTempletu
    }

    createTempletu(templetuDTO: TempletuDTO): ITempletu {
        this.InitiateWeiging.push(templetuDTO)
        return templetuDTO
    }
    ...
    deleteTempletu(id: string) {
        this.InitiateWeiging = this.InitiateWeiging.filter(templetu => templetu.id !== id)
    }
}
