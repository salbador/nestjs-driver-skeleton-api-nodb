...
export class SetCheckinService {
    ...
    getSetCheckin(): ITempletu[] {
        return this.SetCheckin
    }

    getTempletu(id: string): ITempletu {
        const foundedTempletu = this.SetCheckin.find(templetu => templetu.id === id)
        return foundedTempletu
    }

    createTempletu(templetuDTO: TempletuDTO): ITempletu {
        this.SetCheckin.push(templetuDTO)
        return templetuDTO
    }
    ...
    deleteTempletu(id: string) {
        this.SetCheckin = this.SetCheckin.filter(templetu => templetu.id !== id)
    }
}
