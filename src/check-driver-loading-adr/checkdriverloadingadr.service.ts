...
export class CheckDriverLoadingAdrService {
    CheckDriverLoadingAdr: ITempletu[] = [
        { id: '1', 
        lastName: "John",
        firstName: "Doe", 
    ...
    getCheckDriverLoadingAdr(): ITempletu[] {
        return this.CheckDriverLoadingAdr
    }

    getTempletu(id: string): ITempletu {
        const foundedTempletu = this.CheckDriverLoadingAdr.find(templetu => templetu.id === id)
        return foundedTempletu
    }

    createTempletu(templetuDTO: TempletuDTO): ITempletu {
        this.CheckDriverLoadingAdr.push(templetuDTO)
        return templetuDTO
    }
    ...
    deleteTempletu(id: string) {
        this.CheckDriverLoadingAdr = this.CheckDriverLoadingAdr.filter(templetu => templetu.id !== id)
    }
}
